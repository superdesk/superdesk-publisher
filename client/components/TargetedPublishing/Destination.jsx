...
class Destination extends Component {
    constructor(props) {
        ...
        if (props.rule || props.site) {
            ...
            if (props.site) {
                ...
                destination.content_lists = props.rule.content_lists ? props.rule.content_lists : [];
                ...
                subdomain = props.rule.tenant.subdomain ? props.rule.tenant.subdomain : '';
    ...
    getContentLists = () => {
        ...
        return axios.get(this.state.apiUrl + 'content/lists/', {headers: this.props.apiHeader, params: {limit: 1000}})
            .then(res => {
    ...
    render() {
        ...
        let siteDomain = this.state.subdomain ? this.state.subdomain + '.' + this.state.domainName : this.state.domainName;
        let showSaveBar = true;
        let disableSave = this.state.destination && !this.state.destination.route && !this.state.hasOutputChannel ? true : false;
        const destination = {...this.state.destination};
        let contentListsNames = '';

        if (this.props.rule && _.isEqual(this.state.destination, this.state.originalDestination)) {
            showSaveBar = false;
        }
        ...
        if (this.props.rule) {
            if (this.props.rule.tenant.output_channel) {
                ...
                publishRoute = this.state.destination.is_published_fbia ? this.props.rule.route.name + ', Facebook' : this.props.rule.route.name;
            }
        }
                ...
                <div className="sd-collapse-box__content-wraper">
                    <div className="sd-collapse-box__content">
                        <div className="sd-collapse-box__tools sd-collapse-box__tools--flex">
                            <a className="sd-collapse-box__collapse-btn" onClick={this.toggleOpen}>
                                <span className="icn-btn"><i className="icon-chevron-up-thin"></i></span>
                            </a>
                            {this.props.rule &&
                                <a onClick={this.delete}>
                                    <span className="icn-btn"><i className="icon-trash"></i></span>
                                </a>
                            }
                        </div>
                        {showSaveBar &&
                            <SaveBar save={() => this.save()} cancel={() => this.props.cancel()} isDisabled={disableSave}/>
                        }
                        ...
                        {!this.state.hasOutputChannel &&
                            <div className="form__row">
                                <RouteSelect
                                    ...
                                    selectedRouteId={this.state.destination.route ? this.state.destination.route : ''}
                                />
                            </div>
                        }
                        <OptionSwitches
                            ...
                            onChange={(value, fieldName) => this.handleSwitchChange(value, fieldName)}
                        />
