...
jest.mock("moment", () => () => ({
  fromNow: () => "1 day ago",
  format: () => "2020–01–01T12:00:00+00:00",
}));
...
describe("Output/ArticleItem", () => {
  it("renders correctly", () => {
    const { container } = render(
      ...
      >
        <ArticleItem item={item} style={{}} onRemove={jest.fn()} />
      </Store.Provider>
    );
  ...
  it("opens confirmation modal and fires remove action", async () => {
    ...
    const { container, getByText } = render(
      ...
      >
        <ArticleItem item={item} style={{}} onRemove={onRemove} />
      </Store.Provider>
    );
    ...
    const removeButton = await waitForElement(() => getByText("Remove"));
