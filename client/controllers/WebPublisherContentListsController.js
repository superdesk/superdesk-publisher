...
export function WebPublisherContentListsController(
  ...
  class WebPublisherContentLists {
    ...
    setTenant(site) {
      ...
      this._refreshLists().then(() => {
        if ($route.current.params._list) {
          let list = _.find(
            $scope.lists,
            list => list.id === parseInt($route.current.params._list)
          );
    ...
    saveList() {
      ...
      $scope.loading = true;
      publisher
        .manageList(_.pick($scope.newList, updatedKeys), this.selectedList.id)
    ...
    saveManualList() {
      ...
      publisher
        .saveManualList(
          {
            items: $scope.newList.updatedItems,
            updated_at: $scope.newList.updated_at
          },
          $scope.newList.id
        )
        .then(savedList => {
    ...
    deleteList(id) {
      modal
        .confirm(gettext("Please confirm you want to delete list."))
        .then(() => {
          $scope.loading = true;
    ...
    reloadSelectedList() {
      this._refreshLists().then(() => {
        let currentList = $scope.lists.find(
          list => list.id === $scope.newList.id
        );
    ...
    loadMoreTenantArticles() {
      if (
        this.tenantArticles.loading ||
        this.tenantArticles.page >= this.tenantArticles.totalPages
      ) {
        return;
      }
    ...
    removeFromList(index) {
      ...
      selectedItemId = deletedItem[0].content
        ? deletedItem[0].content.id
        : deletedItem[0].id;
      updatedItem = _.find($scope.newList.updatedItems, {
        content_id: selectedItemId
      });
      if (!updatedItem) {
        $scope.newList.updatedItems.push({
          content_id: selectedItemId,
          action: "delete"
        });
      } else if (updatedItem.action === "move") {
        // removes old one and adds delete action
        $scope.newList.updatedItems.splice(
          $scope.newList.updatedItems.indexOf(updatedItem),
          1
        );
        $scope.newList.updatedItems.push({
          content_id: selectedItemId,
    ...
    onDrop(list, item, index) {
      ...
      let itemAction = _.find(
        list.items,
        item =>
          (item.id === selectedItemId && !item.content) ||
          (item.content && item.content.id === selectedItemId)
      )
        ? "move"
        : "add";

      list.updatedItems.push({
        content_id: selectedItemId,
        action: itemAction,
        position: index
      });
    ...
    updatePositions(list) {
      for (let i = 0; i < list.updatedItems.length; i++) {
        let itemInList =
          _.find(list.items, {
            content: { id: list.updatedItems[i].content_id }
    ...
    _queryArticles() {
      ...
      publisher
        .queryTenantArticles(this.tenantArticles.params)
        .then(response => {
