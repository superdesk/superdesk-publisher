...
export function WebPublisherOutputController(
  ...
  class WebPublisherOutput {
    constructor() {
      ...
      this.filterOpen = $window.localStorage.getItem("swpOutputFilterOpen")
        ? JSON.parse($window.localStorage.getItem("swpOutputFilterOpen"))
        : false;
      this.routes = [];
      this.advancedFilters = {};
      this.listType = $window.localStorage.getItem("swpOutputListType")
    ...
    websocketOpen() {
      ...
      this.ws = new WebSocket(
        `${protocol}://${domain}${port}${path}?token=` + publisher.getToken()
      );
    ...
    shouldItemMarkedUnpublished(destination) {
      ...
      return (
        this.publishedDestinations[destination.tenant.code].route.id ===
        this.newDestinations[destination.tenant.code].route.id
      );
    }
    ...
    publishingRemoveDestination(destination) {
      ...
      this.publishingAvailableSites.push({
        code: destination.tenant.code,
        name: destination.tenant.name
      });
    }
    ...
    openPublish(article, action = "publish") {
      ...
      publisher.queryRelatedArticlesStatus(article.id).then(response => {
      ...
      angular.forEach(article.articles, item => {
        if (item.route) {
          ...
          if (item.status == "published") {
            ...
            this.publishedDestinations[item.tenant.code].live_url =
    ...
    unPublishArticle() {
      ...
      publisher
        .unPublishArticle({ tenants: tenants }, this.selectedArticle.id)
        .then(() => {
          this.closePublish();
          this.closePreview();
    ...
    openArticlePreview(routeId, site) {
      ...
      this.previewArticleSrc = $sce.trustAsResourceUrl(
        this.previewArticleUrls.regular
      );
    ...
    saveMetaData() {
      ...
      publisher.saveArticleMetaData(
        metadata,
        this.metaDataSelectedDestination.slug
      );
    }
    ...
    uploadMetaImage(files, key = "meta_media_file") {
      if (files && files.length) {
        ...
        if (!metaFile.$error) {
          ...
          publisher
            .uploadMetaImage(data, this.metaDataSelectedDestination.slug)
            .then(response => {
    ...
    _isAnythingToUnpublish() {
      let destinations = _.pickBy(
        this.newDestinations,
        (value, key) => value.unpublish
      );
