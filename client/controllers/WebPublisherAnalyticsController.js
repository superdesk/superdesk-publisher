 ...
 */
WebPublisherAnalyticsController.$inject = [
  "$scope",
  "publisher",
  "modal",
  "$route"
];
export function WebPublisherAnalyticsController(
  $scope,
  publisher,
  modal,
  $route
  ...
  class WebPublisherAnalytics {
    ...
    isDay(dayName = "today") {
      if (
        this.advancedFilters.published_before &&
        this.advancedFilters.published_before &&
        this.advancedFilters.published_before ===
          this.advancedFilters.published_after
      ) {
        let day = new moment();

        if (
          dayName === "today" &&
          this.advancedFilters.published_before === day.format("YYYY-MM-DD")
        ) {
          return true;
        }
